...
@mcp.tool()
def get_all_bookings(password: str):
    """
    Get details of all bookings (active and cancelled).
    Requires a 4-digit secret password for access.
    
    Args:
        password (str): The 4-digit password.
    
    Returns:
        dict: All bookings if password is correct, otherwise an error message.
    """
    SECRET_PASSWORD = "6637" 

    if password != SECRET_PASSWORD:
        print("Unauthorized access attempt to bookings!")
        return {"error": "Unauthorized: Invalid password."}

    if not cab_service.bookings:
        print("No bookings found.")
        return {}

    print(f"Retrieved {len(cab_service.bookings)} bookings.")
    return cab_service.bookings
